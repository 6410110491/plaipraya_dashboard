...
router.get('/get_summary_inspector', async (req, res) => {
    try {
        const response = await pool.query('SELECT * FROM summary_inspector');
...
router.post('/insert_summary', async (req, res) => {
    ...
    try {
        const existing = await pool.query(
            `SELECT target, result FROM summary_inspector WHERE kpi = $1 AND a_code = $2`,
            [kpi, a_code]
        );
        ...
        await pool.query(
            `DELETE FROM summary_inspector WHERE kpi = $1 AND a_code = $2`,
            [kpi, a_code]
        );

        await pool.query(
            `INSERT INTO summary_inspector (a_code, a_name, target, result, percent, kpi) 
             VALUES ($1, $2, $3, $4, $5, $6)`,
