...
router.get('/get_summary_mou', async (req, res) => {
    try {
        const response = await pool.query('SELECT * FROM SUMMARY_MOU SM ');
...
router.post('/insert_summary', async (req, res) => {
    ...
    try {
        const existing = await pool.query(
            `SELECT target, result FROM summary_mou WHERE kpi = $1 AND a_code = $2`,
            [kpi, a_code]
        );
        ...
        await pool.query(
            `DELETE FROM summary_mou WHERE kpi = $1 AND a_code = $2`,
            [kpi, a_code]
        );
